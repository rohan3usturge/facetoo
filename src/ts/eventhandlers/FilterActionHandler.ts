...
export class FilterActionHandler implements IEventHandler {
    ...
    public RegisterDomHandler(): void {
        this.element.on("change", ".facet-body .facet-item-description .facet-value-checkbox", (event) => {
            ...
            const value = element.val().toString().split(":");
            const action = element.is(":checked") ? FilterActionType.Add : FilterActionType.Minus;
            this.configStore.Options.onFilterChange(value[0], value[1], action);
        });
        this.element.on("click", ".facet-subheader .facet-labels .label .close", (event) => {
            const element = jQuery(event.target).parent();
            const value = element.attr("data-filter-entry").toString().split(":");
